...
func (p *TradeProcessor) ProcessTransaction(lcm xdr.LedgerCloseMeta, transaction ingest.LedgerTransaction) (err error) {
	...
	trades, err := p.extractTrades(lcm.V0.LedgerHeader, transaction)
	if err != nil {
		return err
	}
	...
	for _, trade := range trades {
		...
		p.assetLoader.GetFuture(trade.boughtAsset.String())
		p.assetLoader.GetFuture(trade.soldAsset.String())
	}
...
func (p *TradeProcessor) Commit(ctx context.Context, session db.SessionInterface) error {
	if len(p.trades) == 0 {
		return nil
	}
	...
	for _, trade := range p.trades {
		row := trade.row
		if len(trade.sellerAccount) > 0 {
			id, err := p.accountLoader.GetNow(trade.sellerAccount)
			if err != nil {
				return err
			}
			row.BaseAccountID = null.IntFrom(id)
		}
		if len(trade.buyerAccount) > 0 {
			id, err := p.accountLoader.GetNow(trade.buyerAccount)
			if err != nil {
				return err
			}
		...
		if len(trade.liquidityPoolID) > 0 {
			id, err := p.lpLoader.GetNow(trade.liquidityPoolID)
			if err != nil {
				return err
			}
		...
		var err error
		row.BaseAssetID, err = p.assetLoader.GetNow(trade.soldAsset.String())
		if err != nil {
			return err
		}
		row.CounterAssetID, err = p.assetLoader.GetNow(trade.boughtAsset.String())
		if err != nil {
			return err
		}
