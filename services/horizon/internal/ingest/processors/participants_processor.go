...
func (p *ParticipantsProcessor) ProcessTransaction(lcm xdr.LedgerCloseMeta, transaction ingest.LedgerTransaction) (err error) {
	err = p.addTransactionParticipants(lcm.LedgerSequence(), transaction)
	if err != nil {
		return err
	}

	err = p.addOperationsParticipants(lcm.LedgerSequence(), transaction)
	if err != nil {
		return err
	}
